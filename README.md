...
| --- | --- | --- |
| macOS (Apple Silicon) | `Switchelp_0.1.1_aarch64.dmg` | Signed with a Developer ID but **not notarized**: use **right-click → Open**, or run `xattr -dr com.apple.quarantine /Applications/Switchelp.app` once |
| macOS (Apple Silicon) | `Switchelp-0.1.1-arm64.zip` | Same as above; unzip and drag `Switchelp.app` into `/Applications` |
| Windows (x64) | `Switchelp_0.1.1_x64-setup.exe` (NSIS installer)<br>`Switchelp_0.1.1_x64_en-US.msi` | Unsigned, so SmartScreen reports "Unknown publisher" — choose "Run anyway" |
