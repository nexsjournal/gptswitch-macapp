...
use switch_core::{
    domain::{credential::Credential, error::ErrorCode, ids::{CatalogAlias, CredentialId, ModelId, ProviderId},
        model::Model, provider::{AuthKind, Protocol, Provider}},
    storage::{Repository, SqliteRepository},
};

fn provider(id: &str) -> Provider {
    Provider::draft(ProviderId::new(id), "测试供应商", "https://example.test/v1", Protocol::Responses,
        AuthKind::ApiKey, "2026-09-18T00:00:00Z").unwrap()
}
fn model(id: &str, upstream: &str, alias: &str) -> Model {
    Model::draft(ModelId::new(id), ProviderId::new("p"), upstream, "测试模型",
        CatalogAlias::parse(alias).unwrap(), "2026-09-18T00:00:00Z").unwrap()
}
fn credential(id: &str, provider: &str) -> Credential {
    Credential::create(CredentialId::new(id), ProviderId::new(provider), "测试 Key",
        "synthetic-secret-never-in-sqlite", "2026-09-18T00:00:00Z").unwrap()
}
...
fn persists_entities_across_reopen_without_secret_material() {
    ...
    let path = dir.path().join("metadata.sqlite");
    {
        ...
        repo.save_model(model("m", "Vendor/模型-X", "gs/p/m"), 0).unwrap();
    }
    let repo = SqliteRepository::open(&path).unwrap();
    assert_eq!(repo.list_providers().unwrap().len(), 1);
    assert_eq!(repo.list_models().unwrap()[0].upstream_id, "Vendor/模型-X");
    assert_eq!(repo.list_credentials(&ProviderId::new("p")).unwrap().len(), 1);
...
fn concurrent_connections_cannot_overwrite_a_winning_edit() {
    ...
    let handles: Vec<_> = [a, b].into_iter().enumerate().map(|(index, repository)| {
    ...
    assert_eq!(results.iter().filter(|r| r.as_ref().is_err_and(|e| e.code == ErrorCode::Conflict)).count(), 1);
...
fn model_identity_uses_inherited_protocol_and_is_case_sensitive() {
    ...
    repo.save_provider(provider("p"), 0).unwrap();
    repo.save_model(model("a", "Vendor/X", "gs/p/a"), 0).unwrap();
...
fn renamed_alias_is_released_and_parent_references_are_protected() {
    let repo = SqliteRepository::in_memory().unwrap();
    assert!(repo.save_model(model("missing", "x", "gs/p/missing"), 0).is_err());
    repo.save_provider(provider("p"), 0).unwrap();
...
fn keys_cannot_cross_providers_or_be_deleted_while_selected() {
    ...
    let mut current = repo.get_credential(&CredentialId::new("mine")).unwrap().unwrap();
    current.replace_secret("new-synthetic-secret", "now").unwrap();
