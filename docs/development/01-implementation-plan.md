...
以下任务按 1 名熟悉 Rust/React 的开发者、可使用双平台真机估算，不含签名证书办理时间，也不是交付承诺。

**状态更新（2026-09-18）**：计划中的一期主体已落地——`src/`（前端）、`crates/switch-core/`（核心）、`src-tauri/`（外壳）均已创建，本文档保留为分期与门槛的原始依据；实现进度见仓库根 README 与 `docs/README.md`。
